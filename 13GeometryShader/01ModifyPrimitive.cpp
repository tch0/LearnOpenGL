...
void main()
{
    // move vertices outward along the normal
    vec4 triangleNormal = vec4((varyingNormal[0] + varyingNormal[1] + varyingNormal[2]) / 3.0, 1.0);
    for (int i = 0; i < 3; i++)
    {
        gl_Position = projMatrix * (gl_in[i].gl_Position + normalize(triangleNormal) * 1.0);
...
{
    Utils::Renderer renderer("01ModifyPrimitive");
